"""
Q-PEAK Final_ET_LGBM_Huber_PhysicsKAN_CausalPIMS

主要內容：

統一基準模型使用的特徵：
Cycle_Index、Cycle_log、Cum_Ah、Cum_Energy、Cum_Ah_log1p、Cum_Energy_log1p
QGWO 的最佳化目標設定為最終報告輸出full_causal_pims
QGWO 會調整 lambda_KAN，也就是 KAN 殘差修正模組的融合權重。
保留 Causal Tau 與 train-only cleaning：
Causal Tau 僅使用目前與過去循環資訊；
train-only cleaning 則只使用訓練資料估計清理參數，以避免資料洩漏。
"""
...
# Thesis-ready plotting config. These affect only report figures and do not
# change training, tuning, model structure, or predictions.
FIG_DPI = 320
THESIS_TITLE_FONTSIZE = 16
THESIS_AXIS_LABEL_FONTSIZE = 14
THESIS_TICK_FONTSIZE = 12
THESIS_LEGEND_FONTSIZE = 12
THESIS_ANNOT_FONTSIZE = 11
...
KEEP_FIGURE_FILES = {
    ...
    "xai_fig_foldlocal_feature_importance_seed42.png",
    "xai_fig_permutation_importance_seed42.png",
    "xai_fig_ale_effect_range_summary_seed42.png",
    "xai_fig_ale_1d_Cycle_Index_seed42.png",
    "xai_fig_ale_1d_Cycle_log_seed42.png",
    "xai_fig_ale_1d_CC_Ratio_EMA_seed42.png",
    "xai_fig_ale_1d_Cum_Ah_log1p_seed42.png",
}
...
def _apply_axis_text(ax, title=None, xlabel=None, ylabel=None):
    """Apply thesis-ready font sizes to a matplotlib Axes.

    This is a reporting-only helper. It does not affect training, tuning,
    model structure, predictions, metrics, or PIMS.
    """
    if title is not None:
        ax.set_title(title, fontsize=THESIS_TITLE_FONTSIZE, pad=10)
    if xlabel is not None:
        ax.set_xlabel(xlabel, fontsize=THESIS_AXIS_LABEL_FONTSIZE)
    if ylabel is not None:
        ax.set_ylabel(ylabel, fontsize=THESIS_AXIS_LABEL_FONTSIZE)
    ax.tick_params(axis="x", labelsize=THESIS_TICK_FONTSIZE)
    ax.tick_params(axis="y", labelsize=THESIS_TICK_FONTSIZE)


def _fit_foldlocal_explainer(raw_with_id: pd.DataFrame, seed: int = 42):
    """Train a diagnostic-only fold-local LightGBM explainer.

    The split is the same seed-specific outer GroupShuffleSplit used for
    reporting/XAI. This helper only supports post-hoc explanation figures and
    does not affect Q-PEAK training or final predictions.
    """
    ...
    if len(full_feats) == 0:
        raise ValueError("No explanatory features available for XAI reports.")

    sc = StandardScaler()
    Xtr = sc.fit_transform(dtr[full_feats].values)
    Xte = sc.transform(dte[full_feats].values)
    ytr = dtr["RUL"].values.astype(float)
    yte = dte["RUL"].values.astype(float)
...
def _ale_1d_numeric(model, X: np.ndarray, feat_idx: int, bins: int = 20):
    """Compute a simple 1D ALE curve for one numeric feature."""
    X = np.asarray(X, dtype=float)
    xcol = X[:, feat_idx].astype(float)
    finite = np.isfinite(xcol)
    xcol = xcol[finite]
    Xf = X[finite].copy()
    if len(xcol) < 10:
        raise ValueError("Too few samples for ALE.")

    edges = np.unique(np.quantile(xcol, np.linspace(0.0, 1.0, bins + 1)))
    if len(edges) < 3:
        raise ValueError("Insufficient unique ALE bin edges.")

    centers, effects, counts = [], [], []
    for lo, hi in zip(edges[:-1], edges[1:]):
        if hi <= lo:
            continue
        if hi == edges[-1]:
            mask = (xcol >= lo) & (xcol <= hi)
        else:
            mask = (xcol >= lo) & (xcol < hi)
        if not np.any(mask):
            continue
        ...
        X_low[:, feat_idx] = lo
        X_high[:, feat_idx] = hi
        delta = model.predict(X_high) - model.predict(X_low)
        centers.append((lo + hi) / 2.0)
        effects.append(float(np.mean(delta)))
        counts.append(int(np.sum(mask)))

    if len(centers) < 2:
        raise ValueError("ALE centers are insufficient.")

    effects = np.asarray(effects, dtype=float)
    counts = np.asarray(counts, dtype=float)
    ale = np.cumsum(effects)
    ale = ale - np.average(ale, weights=counts)
    return np.asarray(centers, dtype=float), ale, counts


def prune_output_figures(out_dir: str):
    """Remove extra PNG figures, keeping only the thesis-selected list."""
    if not os.path.exists(out_dir):
        return
    for fname in os.listdir(out_dir):
        if fname.lower().endswith(".png") and fname not in KEEP_FIGURE_FILES:
            try:
...
def make_essential_figures(five_res: Dict[str, Any], lobo_res: Dict[str, Any], ablation_summary: pd.DataFrame, out_dir: str):
    ...
    fig, ax = plt.subplots(figsize=(8.6, 5.4), dpi=FIG_DPI)
    ax.plot(seed_df["Seed"], seed_df["PrePIMS_RMSE"], marker="o", label="Pre-PIMS")
    ax.plot(seed_df["Seed"], seed_df["OfflinePIMS_RMSE"], marker="s", label="Offline PIMS")
    ax.plot(seed_df["Seed"], seed_df["CausalPIMS_RMSE"], marker="^", label="Causal PIMS")
    _apply_axis_text(ax, title=f"5-seed Group Split RMSE - {MODEL_VARIANT}", xlabel="Seed", ylabel="Test RMSE")
    ...
    fig, ax = plt.subplots(figsize=(12.0, 5.8), dpi=FIG_DPI)
    ax.bar(x, vals, edgecolor="black")
    ax.axhline(np.mean(vals), linestyle="--", label=f"Mean={np.mean(vals):.2f}")
    ax.axhline(np.median(vals), linestyle=":", label=f"Median={np.median(vals):.2f}")
    ax.set_xticks(x)
    ax.set_xticklabels(lobo_df["Battery_Label"].values, fontsize=THESIS_TICK_FONTSIZE)
    _apply_axis_text(ax, title=f"LOBO RMSE by Battery - {MODEL_VARIANT}", xlabel="Battery", ylabel="RMSE")
    ax.legend(fontsize=THESIS_LEGEND_FONTSIZE)
    fig.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(9.8, 6.2), dpi=FIG_DPI)
    y = np.arange(len(abl))
    bars = ax.barh(y, abl["Mean_RMSE"].values.astype(float), edgecolor="black")
    ax.set_yticks(y)
    ax.set_yticklabels(abl["Stage"].values, fontsize=THESIS_TICK_FONTSIZE)
    _apply_axis_text(ax, title=f"Stage Ablation - {MODEL_VARIANT}", xlabel="Average Test RMSE", ylabel="Stage")
    ax.grid(axis="x", linestyle=":", alpha=0.35)
    xmax = float(np.max(abl["Mean_RMSE"].values.astype(float)))
    ax.set_xlim(0, xmax * 1.16)
    for yi, v in enumerate(abl["Mean_RMSE"].values.astype(float)):
        ax.text(v + xmax * 0.01, yi, f"{v:.3f}", va="center", fontsize=THESIS_ANNOT_FONTSIZE)
    fig.tight_layout()
    ...
    try:
        ...
        if not st.empty:
            ...
            fig, ax = plt.subplots(figsize=(12.6, 6.0), dpi=FIG_DPI)
            ax.bar(x - width / 2, train_vals, width, edgecolor="black", label="Train")
            ax.bar(x + width / 2, test_vals, width, edgecolor="black", label="Test")
            ax.set_xticks(x)
            ax.set_xticklabels(stages, rotation=28, ha="right", fontsize=THESIS_TICK_FONTSIZE)
            _apply_axis_text(ax, title=f"Train/Test RMSE by Stage - {MODEL_VARIANT}", xlabel="Stage", ylabel="Mean RMSE")
            ax.legend(fontsize=THESIS_LEGEND_FONTSIZE)
            fig.tight_layout()
            fig.savefig(os.path.join(out_dir, "fig_group_split_5seed_stage_train_test_rmse.png"), dpi=FIG_DPI, bbox_inches="tight")
            plt.close(fig)
    ...
    try:
        ...
        if not pv.empty:
            ...
            fig, ax = plt.subplots(figsize=(10.2, 5.8), dpi=FIG_DPI)
            ax.bar(np.arange(len(pv)), pv["Violations"].values.astype(float), edgecolor="black")
            ax.set_xticks(np.arange(len(pv)))
            ax.set_xticklabels(labels, rotation=18, ha="right", fontsize=THESIS_TICK_FONTSIZE)
            _apply_axis_text(ax, title="Monotonicity Violations Before/After PIMS", ylabel="Mean violation count")
            fig.tight_layout()
...
def plot_correlation_heatmap(df: pd.DataFrame, cols: List[str], title: str, save_path: str, max_cols: int = 30):
    ...
    n = len(cols)
    side = max(10.5, 0.42 * n)
    fig, ax = plt.subplots(figsize=(side, side), dpi=FIG_DPI)
    im = ax.imshow(corr.values, aspect="auto")
    cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.ax.tick_params(labelsize=THESIS_TICK_FONTSIZE)
    ax.set_xticks(np.arange(len(cols)))
    ax.set_yticks(np.arange(len(cols)))
    ax.set_xticklabels(cols, rotation=90, fontsize=max(10, THESIS_TICK_FONTSIZE - 1))
    ax.set_yticklabels(cols, fontsize=max(10, THESIS_TICK_FONTSIZE - 1))
    _apply_axis_text(ax, title=title)
    fig.tight_layout()
...
def plot_prediction_trajectories(out_dir: str):
    """
    Plot LOBO trajectories for all batteries in a multi-panel report figure,
    plus residual diagnostics. Requires SAVE_PREDICTIONS=True.
    """
    ...
    if len(folds) > 0:
        ...
        fig, axes = plt.subplots(nrows, ncols, figsize=(16.0, max(4.0 * nrows, 8.0)), dpi=FIG_DPI)
        axes = np.atleast_1d(axes).ravel()
        for ax, fold in zip(axes, folds):
            g = df[df["Fold"] == fold].copy().sort_values("Cycle_Index")
            ax.plot(g["Cycle_Index"], g["True_RUL"], label="True RUL", linewidth=1.8)
            if "Pred_PrePIMS" in g.columns:
                ax.plot(g["Cycle_Index"], g["Pred_PrePIMS"], label="Pre-PIMS", linewidth=1.2)
            if "Pred_CausalPIMS" in g.columns:
                ax.plot(g["Cycle_Index"], g["Pred_CausalPIMS"], label="Causal PIMS", linewidth=1.4)
            _apply_axis_text(ax, title=f"{fold}", xlabel="Cycle Index", ylabel="RUL")
        ...
        fig.suptitle("LOBO Battery-wise RUL Prediction Trajectories", fontsize=THESIS_TITLE_FONTSIZE + 1, y=0.995)
    ...
    if {"True_RUL", "Pred_CausalPIMS"}.issubset(df.columns):
        ...
        fig, ax = plt.subplots(figsize=(8.8, 5.8), dpi=FIG_DPI)
        ax.hist(resid, bins=50, edgecolor="black")
        _apply_axis_text(ax, title="LOBO Residual Distribution - Causal PIMS", xlabel="Residual = True RUL - Predicted RUL", ylabel="Count")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, "fig_lobo_residual_histogram.png"), dpi=FIG_DPI, bbox_inches="tight")
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(8.8, 5.8), dpi=FIG_DPI)
        ax.scatter(df["Pred_CausalPIMS"], resid, s=10, alpha=0.35)
        ax.axhline(0, linestyle="--", linewidth=1.5)
        _apply_axis_text(ax, title="LOBO Residual vs Predicted RUL - Causal PIMS", xlabel="Predicted RUL", ylabel="Residual")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, "fig_lobo_residual_vs_predicted.png"), dpi=FIG_DPI, bbox_inches="tight")
        plt.close(fig)



def plot_qgwo_gate_weight_figures(five_res: Dict[str, Any], lobo_res: Dict[str, Any], out_dir: str):
    """Plot QGWO branch-gate weights. Only the 5-seed figure is kept."""
    df = five_res.get("gate_df", pd.DataFrame())
    if df.empty:
        return
    ...
    fig, ax = plt.subplots(figsize=(9.5, 5.8), dpi=FIG_DPI)
    for c in weight_cols:
        vals = plot_df[c].fillna(0).values.astype(float)
        ax.bar(x, vals, bottom=bottom, label=c.replace("W_", ""), edgecolor="black")
        bottom += vals
    labels = plot_df["Fold"].values if "Fold" in plot_df.columns else x
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=0, fontsize=THESIS_TICK_FONTSIZE)
    _apply_axis_text(ax, title="QGWO Gate Weights - 5 Seeds", xlabel="Seed", ylabel="QGWO Gate Weight")
    ax.legend(fontsize=THESIS_LEGEND_FONTSIZE)
    fig.tight_layout()
...
def make_fold_local_feature_importance(raw_with_id: pd.DataFrame, out_dir: str, seed: int = 42):
    ...
    try:
        pack = _fit_foldlocal_explainer(raw_with_id, seed=seed)
        fi = pd.DataFrame({"Feature": pack["feature_names"], "Importance": pack["model"].feature_importances_})
        fi = fi.sort_values("Importance", ascending=False)
        safe_to_csv(fi, os.path.join(out_dir, f"foldlocal_lightgbm_feature_importance_seed{seed}.csv"))

        top = fi.head(20).iloc[::-1]
        fig, ax = plt.subplots(figsize=(10.0, 7.2), dpi=FIG_DPI)
        ax.barh(top["Feature"], top["Importance"], edgecolor="black")
        _apply_axis_text(ax, title=f"Fold-local Feature Importance - Seed {seed}", xlabel="LightGBM Feature Importance")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"xai_fig_foldlocal_feature_importance_seed{seed}.png"), dpi=FIG_DPI, bbox_inches="tight")
        plt.close(fig)
...
def make_fold_local_permutation_importance(raw_with_id: pd.DataFrame, out_dir: str, seed: int = 42):
    """Permutation importance on the held-out outer test fold for seed 42."""
    try:
        pack = _fit_foldlocal_explainer(raw_with_id, seed=seed)
        model = pack["model"]
        Xte = pack["Xte"]
        ...
        for j, feat in enumerate(feature_names):
            ...
            rows.append({
                "Feature": feat,
                "PermutationDeltaRMSE_Mean": float(np.mean(deltas)),
                "PermutationDeltaRMSE_Std": float(np.std(deltas)),
            })
        perm_df = pd.DataFrame(rows).sort_values("PermutationDeltaRMSE_Mean", ascending=False)
        safe_to_csv(perm_df, os.path.join(out_dir, f"permutation_importance_seed{seed}.csv"))

        top = perm_df.head(20).iloc[::-1]
        fig, ax = plt.subplots(figsize=(10.0, 7.2), dpi=FIG_DPI)
        ax.barh(top["Feature"], top["PermutationDeltaRMSE_Mean"], xerr=top["PermutationDeltaRMSE_Std"], edgecolor="black", capsize=4)
        _apply_axis_text(ax, title=f"Permutation Importance - Seed {seed}", xlabel="Increase in RMSE after permutation")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"xai_fig_permutation_importance_seed{seed}.png"), dpi=FIG_DPI, bbox_inches="tight")
        plt.close(fig)
    except Exception as e:
        safe_to_csv(pd.DataFrame([{"Error": str(e)}]), os.path.join(out_dir, "permutation_importance_error.csv"))



def make_fold_local_ale_reports(raw_with_id: pd.DataFrame, out_dir: str, seed: int = 42):
    """Create 1D ALE curves and a summary figure for selected features."""
    try:
        pack = _fit_foldlocal_explainer(raw_with_id, seed=seed)
        model = pack["model"]
        Xtr = pack["Xtr"]
        ...
        selected = ["Cycle_Index", "Cycle_log", "CC_Ratio_EMA", "Cum_Ah_log1p"]
        rows = []
        for feat in selected:
            if feat not in name_to_idx:
                continue
            idx = name_to_idx[feat]
            centers, ale, counts = _ale_1d_numeric(model, Xtr, idx, bins=20)
            rows.append({
                "Feature": feat,
                "ALE_Min": float(np.min(ale)),
                "ALE_Max": float(np.max(ale)),
                "ALE_Range": float(np.max(ale) - np.min(ale)),
                "N_Bins": int(len(centers)),
            })
            fig, ax = plt.subplots(figsize=(8.6, 5.6), dpi=FIG_DPI)
            ax.plot(centers, ale, linewidth=2)
            ax.axhline(0, linestyle="--", linewidth=1.2)
            _apply_axis_text(ax, title=f"ALE 1D - {feat} (Seed {seed})", xlabel=feat, ylabel="ALE effect on predicted RUL")
            ax.grid(True, linestyle=":", alpha=0.4)
            fig.tight_layout()
            fig.savefig(os.path.join(out_dir, f"xai_fig_ale_1d_{feat}_seed{seed}.png"), dpi=FIG_DPI, bbox_inches="tight")
            plt.close(fig)
        if rows:
            ale_df = pd.DataFrame(rows).sort_values("ALE_Range", ascending=True)
            safe_to_csv(ale_df, os.path.join(out_dir, f"ale_effect_range_summary_seed{seed}.csv"))
            fig, ax = plt.subplots(figsize=(8.8, 5.8), dpi=FIG_DPI)
            y = np.arange(len(ale_df))
            ax.barh(y, ale_df["ALE_Range"].values.astype(float), edgecolor="black")
            ax.set_yticks(y)
            ax.set_yticklabels(ale_df["Feature"].values, fontsize=THESIS_TICK_FONTSIZE)
            _apply_axis_text(ax, title=f"ALE Effect Range Summary - Seed {seed}", xlabel="ALE range", ylabel="Feature")
            fig.tight_layout()
            fig.savefig(os.path.join(out_dir, f"xai_fig_ale_effect_range_summary_seed{seed}.png"), dpi=FIG_DPI, bbox_inches="tight")
            plt.close(fig)
    except Exception as e:
        safe_to_csv(pd.DataFrame([{"Error": str(e)}]), os.path.join(out_dir, "ale_reports_error.csv"))
...
def save_ch4_posthoc_reports(
    ...
    make_fold_local_permutation_importance(raw_with_id, out_dir, seed=FIVE_SEEDS[0])
    make_fold_local_ale_reports(raw_with_id, out_dir, seed=FIVE_SEEDS[0])
    save_literature_comparison_table(master_df, out_dir)
    save_runtime_summary(start_time, out_dir)
    prune_output_figures(out_dir)
